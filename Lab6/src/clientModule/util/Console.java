...
public class Console {
    ...
    public Request interactiveMode(ResponseCode serverResponseCode) throws IncorrectInputInScriptException {
        ...
        try {
            do {
                try {
                    ...
                    while (fileMode() && !userScanner.hasNextLine()) {
                        ...
                        System.out.println("Return from the script '" + scriptFileNames.pop().getName() + "'!");
                    }
                    if (fileMode()) {
                    ...
                    System.out.println("Error occurred while entering the command!");
                    userCommand = new String[]{"", ""};
                }
                processCode = checkCommand(userCommand[0], userCommand[1]);
            } while (processCode == ProcessCode.ERROR.ERROR && !fileMode() || userCommand[0].isEmpty());
            try {
                switch (processCode) {
                    case OBJECT:
                        RouteLite routeToInsert = generateRouteToInsert();
                        return new Request(userCommand[0], userCommand[1], routeToInsert);
                    case UPDATE_OBJECT:
                        RouteLite routeToUpdate = generateRouteToUpdate();
                        return new Request(userCommand[0], userCommand[1], routeToUpdate);
                    case SCRIPT:
            ...
            System.out.println("The script execution has been interrupted!");
