...
import exceptions.NotInBoundsException;
import run.Main;
    ...
    public Long askLong(String coordinateName, Long minValue, Long maxValue) throws IncorrectInputInScriptException{
        ...
        while (true) {
            try {
                ...
                Console.printError("Coordinate " + coordinateName + " not recognized!");
                if (fileMode) throw new IncorrectInputInScriptException();
                System.exit(0);
            } catch (NumberFormatException exception) {
                if (patternNumber.matcher(str).matches())
                    Console.printError("Coordinate " + coordinateName + " should be in the range (" + minValue
                            + ";" + maxValue + ")!");
                else
                    Console.printError("Coordinate " + coordinateName + " must be represented by a number!");
                if (fileMode) throw new IncorrectInputInScriptException();
    ...
    public Long askDistance() throws IncorrectInputInScriptException{
        ...
        while (true) {
            try {
                ...
                Console.printError("Distance not recognized!");
                if (fileMode) throw new IncorrectInputInScriptException();
                System.exit(0);
            } catch (NumberFormatException exception) {
                if (patternNumber.matcher(str).matches())
                    Console.printError("Distance must be in range (" + MIN_DISTANCE
                            + ";" + Long.MAX_VALUE + ")!");
                else
                    Console.printError("Distance must be represented by a number!");
                if (fileMode) throw new IncorrectInputInScriptException();
            } catch (NullPointerException | IllegalStateException exception) {
                Console.printError("Unexpected error!");
                System.exit(0);
